...
`default-pose.json` and `default-pose.meshbin` are generated by
`tools/export_ms_human.py` from the vendored MS-Human-700 full-body model.
They are modified/compiled forms of Apache-2.0-licensed model material and are
not relicensed by the repository's MIT license.

Schema version 2 stores each MuJoCo-compiled tendon point explicitly and joins
only consecutive points, matching MuJoCo's own tendon visualizer. Intermediate
site and wrapping-point classifications are retained for selected-path display.

The generated paths include the two transparent bilateral-coordinate
corrections recorded in `models/ms_human_700/SOURCE.md`. They are local
modifications of the Apache-2.0 model, not upstream-endorsed changes.

See `models/ms_human_700/LICENSE` and `models/ms_human_700/SOURCE.md` in the
repository for the license and exact provenance record.

## Articulated right-arm prototype
