...
Assert-True $Index.Contains('seven right-arm controls') 'Calculation-side disclosure is missing'
Assert-True $Index.Contains('15 guided positions') 'Current assessment position count is missing from the interface'
Assert-True $Index.Contains('diagnosis-use-session-storage') 'Session-only privacy choice is missing from the interface'
Assert-True $Index.Contains('diagnosis-use-device-storage') 'Explicit device-storage choice is missing from the interface'
...
Assert-True $Diagnosis.Contains('storageMode: null') 'Assessment storage is not opt-in at startup'
Assert-True $Diagnosis.Contains("chooseStorageMode('session')") 'Session-only privacy choice is not wired'
Assert-True $Diagnosis.Contains("chooseStorageMode('device')") 'Device-storage privacy choice is not wired'
